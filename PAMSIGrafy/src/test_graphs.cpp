...
void readShortestPathResult(std::istream& is, ShortestPathResult& result)
{
    ...
    while(is)
    {
        ...
        if(is)
        {
            ...
            result[endVertexIndex] = VertexResult{cost, vertices};
        }
    }
}
...
void checkShortestPathResult(const ShortestPathResult& result, const ShortestPathResult& refResult)
{
    ...
    for(auto& [refVertexIndex, refValue] : refResult)
    {
        ...
        auto& vertices = it->second.path;
        auto& refVertices = refValue.path;

        INFO("Checking cost == refCost: " << it->second.cost << " == " << refValue.cost);
        REQUIRE(it->second.cost == refValue.cost);
